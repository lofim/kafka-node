  ...
  async.waterfall(
    [
      ...
      function (metadataResponse, callback) {
        ...
        var missingTopics = _.difference(topics, _.keys(metadata));
        if (missingTopics.length) {
          logger.debug('topics without metadata: ', missingTopics);

          return callback(new errors.TopicsNotExistError(missingTopics));
        }

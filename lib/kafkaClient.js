...
KafkaClient.prototype.clearCallbackQueue = function (socket, error) {
  ...
  if (!longpolling) {
    logger.debug('This is beeing executed....');
    Object.keys(queue).forEach(function (key) {
      ...
      if (error) {
        ...
        // This throws an error into all remaining handlers
        // so the client is able to recover if needed
        cb(new errors.SocketHasClosedError('Socket has been closed'));
      }
    });
  }
